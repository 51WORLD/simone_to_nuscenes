    ...
    for index, case in zip(range(len(cases)), cases):
        ...
        for key, value in mapping_dict.items():
            ...
            if key != 'lidar_top':
                ...
                file_path = glob.glob('simone_datasets/*/front')
                front_dir = file_path[0]
                files = os.listdir(front_dir)
                num = len(files)
                ego_get_idx_counter += int(num - 1)
                ...
                file_path = glob.glob('simone_datasets/*/lidar_top')
                lidar_dir = file_path[0]
                files = os.listdir(lidar_dir)
                num = len(files)
                ego_get_idx_counter += int((num - 1) / 2)
